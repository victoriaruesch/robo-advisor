...
firstinput = input("Would you like to look for a stock or cryptocurrency symbol?")
if firstinput == "stock":
  while True:
      ticker = input("Please input one stock  symbol (between 1 and 5 non-numeric characters).")
...
elif firstinput == "cryptocurrency": 
  while True:
      ticker = input("Please input one cryptocurrency symbol (between 1 and 5 non-numeric characters).")
      if len(ticker)<1 or len(ticker)>5 or ticker.isalpha() == False:
          print("Oh, expecting a properly-formed cryptocurrency symbol like 'BTC'. Please try again.")
      else:
        break
  ...
  load_dotenv()
  ALPHAVANTAGE_API_KEY= os.getenv("ALPHAVANTAGE_API_KEY")
  request_url = f"https://www.alphavantage.co/query?function=DIGITAL_CURRENCY_DAILY&symbol={ticker}&market=USD&apikey={ALPHAVANTAGE_API_KEY}"
  response = requests.get(request_url)

  if "Error Message" in response.text:
      exit("Sorry, couldn't find any trading data for that cryptocurrency symbol.")
  ...
  market_days = list(parsed_response["Time Series (Digital Currency Daily)"])
  latest_day = market_days[0]
  latest_close = float(parsed_response["Time Series (Digital Currency Daily)"][latest_day]["4a. close (USD)"])

  daily_highs = []
  daily_lows = []

  for day in market_days:
      daily_high = parsed_response["Time Series (Digital Currency Daily)"][day]["2a. high (USD)"]
      daily_highs.append(float(daily_high))
      daily_low = parsed_response["Time Series (Digital Currency Daily)"][day]["3a. low (USD)"]
      daily_lows.append(float(daily_low))
  ...
  with open(csv_file_path, "w") as csv_file: 
      ...
      writer.writeheader() 
      for day in market_days:
          daily_prices = parsed_response["Time Series (Digital Currency Daily)"][day]
          writer.writerow({
              "timestamp": day,
              "open": (daily_prices["1a. open (USD)"]),
              "high": (daily_prices["2a. high (USD)"]),
              "low":  (daily_prices["3a. low (USD)"]),
              "close": (daily_prices["4a. close (USD)"]),
              "volume": (daily_prices["5. volume"]),
          })
else:
  exit("Please type either 'stock' or 'cryptocurrency'. Try again.")
...
#reason
why_buy = "Consider a buy of " + ticker.upper() + ". After taking into consideration the user's " + risk_tolerance + " risk tolerance, there is a lucrative difference between the stock's latest closing price and recent high." 
why_sell = "Do not buy " + ticker.upper() + ". After taking into consideration the user's " + risk_tolerance + " risk tolerance, the stock's latest closing price is too high compared to its recent high."     
